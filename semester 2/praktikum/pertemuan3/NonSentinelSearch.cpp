#include <iostream>

using namespace std;

void kembaliMenu();
void LinSearch(int x[5], bool found, int nilaiCari) {
    int i = 0;
    for (i = 0; i < 5; i++)
    {
        if (x[i] == nilaiCari) {
            found = true;
        } else {
            i++;
        }
    }

    if (found) {
        cout << nilaiCari << " ditemukan di index ke-" << i << endl;
    } else {
        cout << "tidak ada " << nilaiCari << " dlm array " << endl;
    }
}

bool ulang = false;

int main() {
    int x[5] = {20, 10, 30, 50, 40};
    int opsiMenu, nilaiCari;
    bool found = false;

    do {
		cout << "=== Search ===" << endl;
        cout << "1. Cari Data" << endl;
		cout << "2. Input Data" << endl;
		cout << "3. Keluar" << endl;
		cout << "Pilih Menu: ";
		cin >> opsiMenu;

        switch (opsiMenu) {
            case 1:
                system("cls");
				ulang = false;

                cout << "Nilai yg dicari: ";
                cin >> nilaiCari;

                LinSearch(x, found, nilaiCari);

                break;
            case 2:
                system("cls");
				ulang = false;

                cout << "bonk" << endl;

                break;
            case 3:
                ulang = false;
				system("cls");
				cout << "\nAnda keluar dari program, Terimakasih...\n" << endl;
				exit(0);
                break;
            default:
                ulang = true;
                system("cls");
                cout << "Input menu sesuai opsi...\n" << endl;
                break;
        }

    } while (ulang == true);
    
    return 0;
}

void kembaliMenu() {
	char kembali;

    cout << "\nApakah Anda ingin kembali ke menu utama? (Y/N): ";
	cin >> kembali;
				
    if (kembali == 'y' || kembali == 'Y') {
		ulang = true;
		system("cls");
	} else {
		system("cls");
		cout << "\nAnda keluar dari program, Terimakasih...\n" << endl;
		exit(0);
	}
}
