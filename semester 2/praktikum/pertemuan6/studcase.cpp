...
void shellSort(Product product[], int size, string option);
...
void shellSort(Product product[], int size, string option) {
    for (int gap = size / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < size; i++) {
            ...
            // if (option == "asc") {
            //     while (j >= gap && product[j - gap].weight > temp.weight) {
            //         product[j] = product[j - gap];
            //         j -= gap;
            //     }
            // } else {
            //     while (j >= gap && product[j - gap].weight < temp.weight) {
            //         product[j] = product[j - gap];z
            //         j -= gap;
            //     }
            // }

            while (j >= gap && (option == "asc" ? product[j - gap].weight > temp.weight : product[j - gap].weight < temp.weight)) {
                product[j] = product[j - gap];
                j -= gap;
            }
...
int main(void)
{
    ...
    cout << "Pilih mau sorting (asc/desc) : ";
    getline(cin, option);
    
    // call you sorting function
    shellSort(Array, sizeProduct, option);
